...
bc_num BC::new_num (int length, int scale)
{
  bc_num temp;
...
int BC::compare (bc_num n1, bc_num n2)
{
...
char BC::is_near_zero (bc_num num, int scale)
{
  int  count;
  char *nptr;
...
bc_num BC::_bc_do_add (bc_num n1, bc_num n2, int scale_min)
{
  bc_num sum;
...
bc_num BC::_bc_do_sub (bc_num n1, bc_num n2, int scale_min)
{
  bc_num diff;
...
void BC::sub (bc_num n1, bc_num n2, bc_num *result, int scale_min)
{
  bc_num diff = NULL;
  int cmp_res;
  int res_scale;
...
void BC::add (bc_num n1, bc_num n2, bc_num* result, int scale_min)
{
  bc_num sum = NULL;
  int cmp_res;
  int res_scale;
...
bc_num BC::new_sub_num (int length, int scale, char* value)
{
  bc_num temp;
...
void BC::_bc_simp_mul (bc_num n1, int n1len, bc_num n2, int n2len, bc_num *prod,
	      int full_scale)
{
...
void BC::_bc_shift_addsub (bc_num accum, bc_num val, int shift, int sub)
{
...
*/
void
BC::_bc_rec_mul (bc_num u, int ulen, bc_num v, int vlen, bc_num *prod,
	     int full_scale)
{ 
  bc_num u0, u1, v0, v1;
  int u0len, v0len;
...
void BC::multiply (bc_num n1, bc_num n2, bc_num* prod, int scale)
{
  bc_num pval; 
  int len1, len2;
...
int BC::divide (bc_num n1, bc_num n2, bc_num* quot, int scale)
{
  bc_num qval;
...
int BC::divmod (bc_num num1, bc_num num2, bc_num *quot, bc_num *rem, int scale)
{
  ...
  /* Calculate it. */
  divide (num1, num2, &temp, scale);
  if (quot)
    quotient = copy_num (temp);
  multiply (temp, num2, &temp, rscale);
  sub (num1, temp, rem, rscale);
  free_num(&temp);
...
int
BC::modulo (bc_num num1, bc_num num2, bc_num* result, int scale)
{
...
int
BC::raisemod (std::ostream& irc, bc_num base, bc_num expo, bc_num mod, bc_num *result, int scale)
{
  ...
  if (base->n_scale != 0)
      irc << "non-zero scale in base" << std::endl;

  /* Check the exponent for scale digits. */
  if (exponent->n_scale != 0)
    {
      irc << "non-zero scale in exponent" << std::endl;
      divide (exponent, _one_, &exponent, 0); /*truncate */
    }

  /* Check the modulus for scale digits. */
  if (mod->n_scale != 0)
     irc << "non-zero scale in modulus" << std::endl;

  /* Do the calculation. */
  rscale = MAX(scale, base->n_scale);
  while ( !is_zero(exponent) )
    {
      (void) divmod (exponent, _two_, &exponent, &parity, 0);
      if ( !is_zero(parity) )
	{
	  multiply (temp, power, &temp, rscale);
	  (void) modulo (temp, mod, &temp, scale);
	}

      multiply (power, power, &power, rscale);
      (void) modulo (power, mod, &power, scale);
    }
...
void
BC::raise (std::ostream& irc, bc_num num1, bc_num num2, bc_num *result, int scale)
{
   bc_num temp, power;
   long exponent;
   int rscale;
   ...
   if ((exponent == 0 && (num2->n_len > 1 || num2->n_value[0] != 0))) {
	   irc << "exponent too large in raise" << std::endl;
	   exponent = 0;
   }
   ...
   while ((exponent & 1) == 0)
     {
       pwrscale = 2*pwrscale;
       multiply (power, power, &power, pwrscale);
       exponent = exponent >> 1;
     }
   temp = copy_num (power);
   calcscale = pwrscale;
   ...
   while (exponent > 0)
	 ...
	 multiply (temp, power, &temp, calcscale);
       }
       exponent = exponent >> 1;
     }
   ...
   if (neg)
     {
       divide (_one_, temp, result, rscale);
       free_num(&temp);
     }
   else
     {
...
int
BC::sqrt (std::ostream& irc, bc_num *num, int scale)
{
  ...
  if (cmp_res < 0)
  ...
  else
    {
      ...
      multiply (guess1, point5, &guess1, 0);
      guess1->n_scale = 0;
      raise (irc, guess, guess1, &guess, 0);
      free_num(&guess1);
      cscale = 3;
    }
  ...
  while (!done)
    {
      ...
      divide (*num, guess, &guess, cscale);
      add (guess, guess1, &guess, 0);
      multiply (guess, point5, &guess, cscale);
      sub (guess, guess1, &diff, cscale+1);
...
void
BC::bc_out_long (long val, int size, int space, std::string& output)
{
  char digits[40];
  int len, ix;

  if (space) output.push_back (' ');
...
void
BC::out_num (bc_num num, int o_base, std::string& output, int leading_zero)
{
  char *nptr;
  ...
  if (num->n_sign == MINUS) output.push_back ('-');
  ...
  if (is_zero (num))
	...
	if (num->n_len > 1 || *nptr != 0)
	  for (index=num->n_len; index>0; index--)
	    output.push_back (BCD_CHAR(*nptr++));
	else
	  nptr++;

	if (leading_zero && is_zero (num))
	  output.push_back ('0');
	...
	digits = NULL;
	init_num (&int_part);
	divide (num, _one_, &int_part, 0);
	init_num (&frac_part);
	init_num (&cur_dig);
	init_num (&base);
	sub (num, int_part, &frac_part, 0);
	...
	while (!is_zero (int_part))
	  {
	    modulo (int_part, base, &cur_dig, 0);
	    temp = (stk_rec *) malloc (sizeof(stk_rec));
	    if (temp == NULL) bc_out_of_memory();
	    temp->digit = num2long (cur_dig);
	    temp->next = digits;
	    digits = temp;
	    divide (int_part, base, &int_part, 0);
	  }
	...
	if (digits != NULL)
	  {
		...
		digits = digits->next;
		if (o_base <= 16)
			output.push_back (ref_str[ (int) temp->digit]);
		else
		  bc_out_long (temp->digit, max_o_digit->n_len, 1, output);
		free (temp);
	      }
	  }
	...
	if (num->n_scale > 0)
	  {
	    ...
	    while (t_num->n_len <= num->n_scale) {
	      multiply (frac_part, base, &frac_part, num->n_scale);
	      fdigit = num2long (frac_part);
	      int2num (&int_part, fdigit);
	      sub (frac_part, int_part, &frac_part, 0);
	      if (o_base <= 16)
			  output.push_back (ref_str[fdigit]);
	      else {
		bc_out_long (fdigit, max_o_digit->n_len, pre_space, output);
		pre_space = 1;
	      }
	      multiply (t_num, base, &t_num, 0);
	    }
	    free_num(&t_num);
	  }
...
char* BC::internal_num2str (bc_num num)
{
  char *str, *sptr;
  char *nptr;
  int  index, signch;
...
void
BC::str2num (bc_num* num, char* str, int scale)
{
  int digits, strscale;
  char *ptr, *nptr;
  char zero_int;
